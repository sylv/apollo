/**
 * @types/signale is outdated
 * signale has it's own types but doesn't publish them on npm
 * this was the easiest way to use them :(
 */
...
declare namespace _signale {
  ...
  export interface Constructor {
    new <T extends string = DefaultLogger>(options?: ConstructorOptions<T>): Instance<T>;
  }
  ...
  export type Instance<T extends string = DefaultLogger> = Base<T> & Record<T, LoggerFunction> & Record<DefaultLogger, LoggerFunction>;
}
...
declare namespace signale {
  ...
  export interface SignaleConfiguration extends _signale.InstanceConfiguration {}
  export interface LoggerConfiguration extends _signale.LoggerConfiguration {}
  export interface SignaleConstructorOptions<T extends string = DefaultLogger> extends _signale.ConstructorOptions<T> {}
}
